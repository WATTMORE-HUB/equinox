      ...
      if (s3Bucket && this.hasAwsCredentials()) {
        try {
          ...
          if (config.token) {
            ...
            // Try /collect_data first (persistent across deployments), then fall back to /etc/equinox
            const persistPaths = [
            ...
            for (const { dir, label } of persistPaths) {
              try {
                ...
                console.log(`[BalenaTokenManager] Persisted token to ${label} (${configPath})`);
                break; // Success, don't try other paths
              } catch (persistError) {
                console.warn(`[BalenaTokenManager] Warning: Failed to persist token to ${label}: ${persistError.message}`);
