...
# RTSP stream URL
RTSP_URL = f'rtsp://stream:stream42@{os.getenv("CAMERA_IP")}:554/h264Preview_01_main'
...
ONVIFcamera = ONVIFCamera(os.getenv("CAMERA_IP"), 8000, "stream", "stream42")
...
def snow_check():
    ...
    1. Find aruco codes
        a. check current position
            i. if found, center
            ii. if not found, move 2 seconds to the right
    2. Take picture
    3. Analyze picture's contents
    '''
    cam = get_camera()
    ret, frame = cam.read()
    
    if ret:
        cv2.imwrite("/collect_data/snow_check.jpg", frame, [cv2.IMWRITE_JPEG_QUALITY, 100])
